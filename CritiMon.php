...
require_once 'CritiMonBase.php';
require_once 'CritiMonExceptions.php';

class CritiMon extends CritiMonBase
{
    public function __construct($app_id, $api_key, $version_number)
    {
        parent::__construct($app_id, $api_key, $version_number);
    }
    ...
    public function initialise()
    {
        ...
        if (isset($_COOKIE["SESSIONID"]))
        {
            ...
            $this->cookies["session_id"] = $_COOKIE["session_id"];
        }
        ...
        try
        {
            ...
            if ($err)
            {
                ...
                {
                    error_log("Unable to initialise CritiMon: Error: $err");
                }
            }
            else
            {
                if ($httpcode === 200)
                {
                    ...
                    foreach ($matches[0] as $cookie)
                    {
                        ...
                        if (trim($keyValueCookie[0]) === "DO-LB")
                        {
                            ...
                            setcookie("DO-LB", trim($keyValueCookie[1]), time() + (60 * 10), critimon_url);
                        }
                        if (trim($keyValueCookie[0]) === "SESSIONID")
                        {
                            $this->cookies[trim($keyValueCookie[0])] = trim($keyValueCookie[1]);
                            setcookie("SESSIONID", trim($keyValueCookie[1]), time() + (60 * 10), critimon_url);
                        }
                        if (trim($keyValueCookie[0]) === "session_id")
                        {
                            $this->cookies[trim($keyValueCookie[0])] = trim($keyValueCookie[1]);
                            setcookie("session_id", trim($keyValueCookie[1]), time() + (60 * 10), critimon_url);
                        }
                    }
                ...
                {
                    throw new CritiMonException("CritiMon initialisation failed: $response", $httpcode);
                }
            }
        }
        catch (Exception $ex)
        {
            if (function_exists("error_log"))
            {
                error_log("Unable to initialise critimon. Error: " . $ex->getMessage());
            }
        }
    }
    ...
    public function FatalErrorHandler()
    {
        ...
        if ($error !== NULL && in_array($error['type'], array(E_ERROR, E_PARSE, E_CORE_ERROR, E_CORE_WARNING, E_COMPILE_ERROR, E_COMPILE_WARNING,E_RECOVERABLE_ERROR))) {
            ...
            $postFields["Severity"] = CritiMonSeverity::CRITICAL;
    ...
    public function ErrorHandler($phpErrorType, $message, $filename, $lineno)
    {
            ...
            switch ($phpErrorType)
            {
                ...
                case E_CORE_ERROR:
                    ...
                    $severity = CritiMonSeverity::CRITICAL;
                    break;
                case E_CORE_WARNING:
                    $exceptionType = "PHP_CORE_WARNING";
                    $severity = CritiMonSeverity::MEDIUM;
                    break;
                ...
                case E_COMPILE_WARNING:
                    ...
                    $severity = CritiMonSeverity::MAJOR;
                    break;
     ...
     * @throws CritiMonException
     */
    public function reportCrash($exception, $severity = CritiMonSeverity::MEDIUM, $customProperties = null)
    {
        if (!is_subclass_of($exception, "Exception") && !is_a($exception, "Exception"))
        {
            throw new CritiMonException("The exception object is not a sub class of Exception");
        }
    ...
    private function sendCrashData($postFields)
    {
        ...
        try
        {
            ...
            if ($err)
            {
                throw new CritiMonException("Unable to initialise CritiMon. Error: " . $err);
            }
            else
            {
                if ($httpcode === 200)
                ...
                {
                    throw new CritiMonException("Failed to send crash: $response", $httpcode);
                }
            }
        }
        catch (Exception $ex)
        {
            if (function_exists("error_log"))
            {
                error_log("CritiMon: Failed to send crash. Error: " . $ex->getMessage());
            }
        }
    }
...
abstract class CritiMonSeverity
{
    const LOW = "Low";
    const MEDIUM = "Medium";
    const MAJOR = "Major";
